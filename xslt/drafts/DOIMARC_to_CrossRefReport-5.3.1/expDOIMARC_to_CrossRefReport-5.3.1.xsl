...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0" xmlns="http://www.crossref.org/schema/5.3.1" xmlns:cr="http://www.crossref.org/schema/5.3.1" xmlns:f="http://functions" xmlns:isodates="http://iso" xmlns:local="http://www.local.gov/namespace" xmlns:map="http://www.w3.org/2005/xpath-functions/map" xmlns:marc="http://www.loc.gov/MARC21/slim" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="cr f isodates local marc xd xlink xs xsi" xpath-default-namespace="http://www.loc.gov/marc21/slim">
	...
	<xd:doc scope="stylesheet">
		<xd:desc>
			...
			<xd:p><xd:b>f:createTimestamp()</xd:b>authored by Amanada Xu</xd:p>
	...
	<xsl:import href="ror-api-query.xsl"/>
...
<!-- Templates -->	
	...
	<xsl:template match="/">
		<xsl:result-document version="1.0" method="xml" format="CrossRef" href="{substring-before(base-uri(),tokenize(base-uri(),'/')[last()])}/{replace(base-uri(),'(.*/)(.*)(\.xml)','$2')}_{'mrc2CrossRef'}.xml">
	...
	<xsl:template match="marc:record">
			<report-paper>
				...
				<!-- titles -->
					<titles>
					<xsl:apply-templates select="marc:datafield[@tag='245'] | marc:datafield[@tag='880'][@ind2!='2'][starts-with(marc:subfield[@code='6'],'245')]" mode="title"/>					
					</titles>
				<!-- abstract -->
		...
		<person_name sequence="{f:addSequence(.)}">
			<xsl:attribute name="contributor_role">
				<xsl:choose>
					...
					<xsl:when test="string('primary investigator')">
						<xsl:value-of select="string('author')"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:variable name="invalidValue" select="$role"/>
						<xsl:value-of select="$invalidValue"/>
						<xsl:message terminate="no" select="concat('Warning! Invalid value of: &quot;', $invalidValue, ' &quot; was selected. ', 'Only one of the following values may be selected: author, editor, chair, reviewer, review-assistant, stats-reviewer, reviewer-external, reader, translator.' )"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
	...
	<xsl:template match="marc:datafield[@tag='245'] | marc:datafield[@tag='880'][starts-with(marc:subfield[@code='6'],'245')]" mode="title">
...
<!-- dateIssued -->
	...
	<xsl:template name="dateIssued">
		...
		<xsl:analyze-string select="substring(marc:controlfield[@tag = '008'], 1, 15)" regex="(\d+)(\w)(\d+)">
...
<!-- handle -->
	...
	<xd:doc>
		<xd:desc> 880 - alternate graphic representation - transliteration processing </xd:desc>
	</xd:doc>
	...
	<!-- nameTitleGroup -->
	<xd:doc>
		<xd:desc> rules for applying nameTitleGroup attribute </xd:desc>
	</xd:doc>
