...
<!-- abstract -->
	...
	<xsl:template match="marc:datafield[@tag='264']/marc:subfield[@code='c']" mode="publisher">
		...
		<xsl:choose>
			<xsl:when test="matches($dateString, '^[A-z]')">
				<publication_date media_type="online">					
				<xsl:variable name="monthNum" select="f:monthNumFromName(tokenize($dateString,'\D+'))"/>
				<xsl:variable name="newDateString" select="replace($datestring, '(\D+)(\d+)(.+)',$monthNum'-$2-$3')"/>
				<xsl:value-of select="f:splitDates($newDateString)"/>
				</publication_date>
			</xsl:when>
		<publication_date media_type="online">
			<xsl:copy-of select="f:splitDates($dateString)"/>
		</publication_date>
		</xsl:choose>
	</xsl:template>
